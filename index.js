...
const PATTERNS = [
  ...
  // HIGH — embedded TLS / SSH / key-agreement gaps (2026-06-25 v1.9.2)
...
// ── Fix mode ──────────────────────────────────────────────────────────────────
function writeFixes(findings, targetDir, score) {
  ...
  const lines = [
    ...
    "",
    `> Generated by [QuantumScan](https://quantumscan.io) v${VERSION} · ${today}`,
    `> Risk Score: **${score}/100**`,
    "",
    "## Summary",
    "",
    `Found **${findings.length} finding${findings.length !== 1 ? "s" : ""}** across **${uniqueFiles} file${uniqueFiles !== 1 ? "s" : ""}**.`,
    "",
  ...
  for (const sev of SEV_ORDER) {
    ...
    lines.push(`### ${icon} ${sev.charAt(0).toUpperCase() + sev.slice(1)} (${group.length})`);
    lines.push("");
    for (const f of group) {
      lines.push(`#### \`${f.name}\` — \`${f.file}:${f.line}\``);
      if (f.match) lines.push(`- **Detected:** \`${f.match}\``);
      if (f.alt)   lines.push(`- **Migrate to:** ${f.alt}`);
  ...
  lines.push("");
  lines.push("1. **Critical/High** — replace asymmetric algorithms (RSA, ECDSA, ECDH, DH) with **ML-KEM-768** (key encapsulation, FIPS 203) and **ML-DSA-65** (signatures, FIPS 204).");
  lines.push("2. **Symmetric** — upgrade AES-128 or shorter keys to **AES-256** (Grover halves effective key length).");
  lines.push("3. **Hashes** — replace MD5/SHA-1 with **SHA-256** or **SHA3-256**.");
  lines.push("4. Run full AI analysis at [quantumscan.io](https://quantumscan.io) for per-file migration code diffs.");
  lines.push("");
  lines.push("---");
  lines.push("*Required by NIST FIPS 203/204/205, DORA, NIS2, CNSA 2.0 — deadline 2030.*");

  const outPath = join(targetDir, "QUANTUMSCAN_FIXES.md");
  writeFileSync(outPath, lines.join("\n"), "utf8");
  return outPath;
}

function tryCreatePR(targetDir, fixesPath) {
  // Confirm we are inside a git repository
  let repoRoot;
  try {
    repoRoot = execSync("git rev-parse --show-toplevel", { cwd: targetDir, stdio: ["pipe","pipe","pipe"] })
      .toString().trim();
  } catch { return null; }

  // gh CLI required for PR creation
  try { execSync("gh --version", { stdio: ["pipe","pipe","pipe"] }); }
  catch { return null; }
  ...
  try {
    ...
    execSync(`git push -u origin ${branch}`, { cwd: repoRoot, stdio: ["pipe","pipe","pipe"] });
    const prUrl = execSync(
      `gh pr create --title "fix: post-quantum cryptography migration guide" --body "Adds \`QUANTUMSCAN_FIXES.md\` with per-finding PQC migration guidance.\\n\\nGenerated by [QuantumScan](https://quantumscan.io) — NIST FIPS 203/204/205 deadline 2030." --head ${branch}`,
...
Options:
  ...
  --fix              Write QUANTUMSCAN_FIXES.md with PQC migration guidance;
                     if gh CLI is available, opens a GitHub PR automatically
...
function main() {
  ...
  if (sarifMode) {
    ...
    if (fixMode) {
      const fixesPath = writeFixes(allFindings, targetDir, score);
      if (fixesPath) {
        console.log(`\n${C.cyan}--fix${C.reset}  Migration guide → ${C.bold}${fixesPath}${C.reset}`);
        const prUrl = tryCreatePR(targetDir, fixesPath);
        if (prUrl) {
          console.log(`${C.green}  PR created → ${prUrl}${C.reset}`);
        } else {
          console.log(`${C.dim}  Commit QUANTUMSCAN_FIXES.md to your repo and open a PR to track remediation.${C.reset}`);
          console.log(`${C.dim}  (gh CLI not found or not in a git repo — PR skipped)${C.reset}`);
        }
        console.log("");
      } else {
